...
if nargin < 2
    ...
    %C = [1 1 1;1 0 30;0 1 30;1 2 5;2 1 5];  %Nice one for QBH
    C = [1 1 1;1 0 30;0 1 30;1 2 5;2 1 5];
end
...
if T == 0
    ...
    if (min(TE) < min(RE))
        i = r;
        j = find(TE==min(TE), 1, 'last' );
    else
        i = find(RE==min(RE), 1, 'last' );
        j = c;
    end
else
    if min(size(D)) == 1
